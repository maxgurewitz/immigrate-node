...
Elm.Components.make = function (_elm) {
   ...
   if (_elm.Components.values)
   return _elm.Components.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm),
   $Tasks = Elm.Tasks.make(_elm);
   var navbarLinks = function (model) {
      return function () {
         var pathToLink = function (_v0) {
            return function () {
               switch (_v0.ctor)
               {case "_Tuple2":
                  return A2($Html.li,
                    _L.fromArray([_U.eq(_v0._0,
                    model.path) ? $Html$Attributes.$class("active") : $Html$Attributes.$class("")]),
                    _L.fromArray([A2($Html.a,
                    _L.fromArray([A2($Html$Events.onClick,
                    $Actions.asyncActions.address,
                    $Tasks.setPath(_v0._0))]),
                    _L.fromArray([$Html.text(_v0._1)]))]));}
               _U.badCase($moduleName,
               "between lines 148 and 152");
            }();
         };
         var pathsAndNames = _L.fromArray([{ctor: "_Tuple2"
                                           ,_0: "/"
                                           ,_1: "Home"}
                                          ,{ctor: "_Tuple2"
                                           ,_0: "/immigrate"
   ...
   var navbar = function (model) {
      ...
      _L.fromArray([A2($Html.div,
                   ...
                   _L.fromArray([A2($Html.button,
                                ...
                                _L.fromArray([$Html.text($Constants.companyName)]))]))
                   ,A2($Html.div,
                   _L.fromArray([$Html$Attributes.$class("collapse navbar-collapse")
                                ,A2($Html$Attributes.attribute,
                                "aria-expanded",
                                "false")
                                ,$Html$Attributes.id("js-navbar-collapse")
                                ,A2($Html$Attributes.attribute,
                                "role",
                                "presentation")]),
                   _L.fromArray([navbarLinks(model)]))]))]));
   };
   var notFoundPage = function (model) {
      return $Html.text("404");
   };
   var brk = A2($Html.br,
   _L.fromArray([]),
   _L.fromArray([]));
   var profileFormInput = F3(function (model,
   labelName,
   name) {
      return function () {
         var currentValue = $Maybe.withDefault("")(A2($Dict.get,
         name,
         model.profileForm));
         var updateField = function (text) {
            return A2($Signal.message,
            $Actions.actions.address,
            A2($Actions.ProfileFormChange,
            name,
            text));
         };
         return A2($Html.div,
         ...
         _L.fromArray([A2($Html.label,
                      _L.fromArray([A2($Html$Attributes.attribute,
                                   "for",
                                   name)
                                   ,$Html$Attributes.$class("col-xs-offset-1")]),
                      _L.fromArray([$Html.text(labelName)]))
                      ,A2($Html.input,
                      _L.fromArray([$Html$Attributes.$class("immigration-input col-xs-10 col-xs-offset-1")
   ...
   var pageLayout = F2(function (model,
   content) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("page-container")]),
      _L.fromArray([navbar(model)
                   ,A2($Html.div,
                   _L.fromArray([$Html$Attributes.$class("row")]),
                   _L.fromArray([A2($Html.div,
                   _L.fromArray([$Html$Attributes.$class("panel panel-default col-md-6 col-md-offset-3")]),
                   _L.fromArray([A2($Html.div,
                   _L.fromArray([$Html$Attributes.$class("panel-body row")]),
                   content)]))]))]));
   });
   var homePage = function (model) {
      return A2(pageLayout,
      model,
      _L.fromArray([A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("home")]),
      _L.fromArray([$Html.text(model.field)
                   ,$Html.text("bar")
                   ,A2($Html.button,
                   _L.fromArray([A2($Html$Events.onClick,
                   $Actions.actions.address,
                   $Actions.Update)]),
                   _L.fromArray([$Html.text("button")]))]))]));
   };
   var immigratePage = function (model) {
      return function () {
         var inp = profileFormInput(model);
         return A2(pageLayout,
         model,
         _L.fromArray([A2(inp,
                      "First Name",
                      "firstName")
                      ,A2(inp,"Last Name","lastName")
                      ,A2(inp,"Age","age")
                      ,A2(inp,
                      "Country of Origin",
                      "countryOfOrigin")
                      ...
                      _L.fromArray([A2($Html$Events.onClick,
                                   $Actions.actions.address,
                                   $Actions.SubmitProfileForm)
                                   ,$Html$Attributes.$class("btn btn-default form-submit")
                                   ,A2($Html$Attributes.attribute,
                                   "type",
                                   "submit")]),
   ...
   var aboutPage = function (model) {
      return A2(pageLayout,
      model,
      _L.fromArray([A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("col-xs-offset-1")]),
      _L.fromArray([$Html.text(A2($Basics._op["++"],
                   "At ",
                   A2($Basics._op["++"],
                   $Constants.companyName,
                   " we\'re all about helping you build a better life.")))
                   ,brk
                   ,brk
                   ,$Html.text("If you have had trouble with expensive immigration \n        lawyers and confusing governmental bureaucracies give us a try.  \n        We will make naturalization easy!")]))]));
   };
   var baseLayout = function (content) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("app")]),
      content);
   };
   _elm.Components.values = {_op: _op
                            ,baseLayout: baseLayout
                            ,pageLayout: pageLayout
                            ,homePage: homePage
                            ,profileFormInput: profileFormInput
                            ,immigratePage: immigratePage
                            ,brk: brk
                            ,aboutPage: aboutPage
...
Elm.Main.make = function (_elm) {
   ...
   var view = function (model) {
      return $Components.baseLayout(_L.fromArray([A2($Router.router,
      model.path,
      model)]));
   };
   var asyncPort = Elm.Native.Task.make(_elm).performSignal("asyncPort",
   $Actions.asyncActions.signal);
   var model = A3($Signal.foldp,
   $Update.update,
   $Model.initialModel,
   $Actions.actions.signal);
...
Elm.Model.make = function (_elm) {
   ...
   var initialPath = $Native$Main.initialPath;
   var initialModel = {_: {}
                      ,field: "foo"
                      ,path: initialPath
                      ,profileForm: $Dict.fromList(_L.fromArray([]))};
...
Elm.Router.make = function (_elm) {
   ...
   $Basics = Elm.Basics.make(_elm),
   $Components = Elm.Components.make(_elm);
   var buildRouter = F3(function (routes,
   defaultPage,
   path) {
      return function () {
         switch (routes.ctor)
         {case "::":
            return _U.eq($Basics.fst(routes._0),
              path) ? $Basics.snd(routes._0) : A3(buildRouter,
              routes._1,
              defaultPage,
              path);
            case "[]": return defaultPage;}
         _U.badCase($moduleName,
         "between lines 8 and 11");
      }();
   });
   var router = A2(buildRouter,
   _L.fromArray([{ctor: "_Tuple2"
                 ,_0: "/"
                 ,_1: $Components.homePage}
                ,{ctor: "_Tuple2"
                 ,_0: "/about"
                 ,_1: $Components.aboutPage}
                ,{ctor: "_Tuple2"
                 ,_0: "/immigrate"
                 ,_1: $Components.immigratePage}]),
...
Elm.Tasks = Elm.Tasks || {};
Elm.Tasks.make = function (_elm) {
   "use strict";
   _elm.Tasks = _elm.Tasks || {};
   if (_elm.Tasks.values)
   return _elm.Tasks.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
      ...
      return function () {
         ...
         {case "NoOp": return model;
            ...
            return _U.replace([["path"
                               ,action._0]],
              model);
            case "ProfileFormChange":
            return function () {
                 var updatedForm = A3($Dict.insert,
                 action._0,
                 action._1,
                 model.profileForm);
                 return _U.replace([["profileForm"
                                    ,updatedForm]],
                 model);
              }();
            case "Update":
